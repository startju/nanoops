...
parser.add_argument("--val-device-batch-size", type=int, default=-1, help="per-device batch size for validation eval (-1 = same as train device_batch_size). Larger val batch is safe because there's no backward → no activations, no optim state transient.")
...
val_device_batch_size = args.val_device_batch_size if args.val_device_batch_size > 0 else args.device_batch_size
...
while True:
    ...
    if args.eval_every > 0 and (last_step or step % args.eval_every == 0):
        ...
        eval_steps = args.eval_tokens // (val_device_batch_size * args.max_seq_len * ddp_world_size)
