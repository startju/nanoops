...
**为什么是加法 mask 不是乘法？** 因为 softmax 跟「加法 / $-\infty$」天然兼容，
跟「乘法 / $0$」不兼容。对比三种候选：

| 方案 | 公式 | masked $P_{ij}$ | 行和为 1？ |
|---|---|---|---|
| 加法（用） | $\text{softmax}(S + M),\; M \in \{0, -\infty\}$ | $e^{-\infty}/Z = 0$ ✓ | 是 ✓ |
| 乘法在 $S$ 上 | $\text{softmax}(S \odot m),\; m \in \{0, 1\}$ | $e^0/Z = 1/Z \ne 0$ ✗ | 是，但被掩位置还分到概率 |
| 乘法在 $P$ 上 | $\text{softmax}(S) \odot m$ | $0$ ✓ | 否——行和 $< 1$ |
...
3. **梯度自动消**（下面 backward 推导的核心）：$P_{ij} = 0$ 通过 softmax-backward
   公式本身就传成 $\partial L / \partial S_{ij} = 0$。乘法在 $P$ 上的版本，
   $P_{ij} = 0$ 但归一化已经错了，反向还要补一次 mask。

更深的代数原因：在对数空间里
$\log\text{softmax}(S + M) = (S + M) - \log\sum e^{S + M}$ ——把 mask 加到
logit 上跟 log-sum-exp 归一化是**同一个操作**。同一招让 log-sum-exp 数值稳定、
让 cross-entropy 跟 logsoftmax 可以融合、让 FlashAttention 只存 $L = \log\sum e^S$
就够 backward。乘法 mask 没法套这个代数结构。
