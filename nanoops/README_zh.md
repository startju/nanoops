...
**为什么是加法 mask 不是乘法？** "softmax 之后归 0"最直观的乘法对应是
$M \in \{-\infty, 1\}$ 直接乘到 $S$ 上——但这条路被 $S_{ij}$ 的**符号**
破坏了：

| 方案 | 公式 | masked $P_{ij}$ |
|---|---|---|
| 加法（用） | $\text{softmax}(S + M),\; M \in \{0, -\infty\}$ | 对**任何** $S_{ij} \in \mathbb{R}$，$e^{-\infty}/Z = 0$ ✓ |
| 乘法在 $S$ 上 | $\text{softmax}(S \odot M),\; M \in \{-\infty, 1\}$ | $S_{ij} \cdot (-\infty)$ = $-\infty$ 若 $S_{ij} > 0$，$+\infty$ 若 $S_{ij} < 0$，**NaN** 若 $S_{ij} = 0$ |

Attention scores $S = QK^T / \sqrt{d_k}$ 取**任意实数**——经常为负，也常经过
$0$ 附近。所以乘法-在-$S$ 这条路只要 $S_{ij} \le 0$ 在被掩位置出现（早晚都会出），
整行要么被 $+\infty$ 独吞概率、要么被 NaN 污染。加法 mask 没有这种符号依赖：
$-\infty$ 对于 $\max$ / softmax 是 **任意** $S_{ij}$ 的吸收元。
